...
Handle<Value> HtmlStripFunc(const FunctionCallbackInfo<Value>& args) {
    uint16_t* inBuf = NULL;
    size_t inBufSize = 0;
    if (args.Length() >= 2) {
...
Handle<Value> HtmlEntitiesDecodeFunc(const FunctionCallbackInfo<Value>& args) {
...
Handle<Value> AccentedCharsNormalizeFunc(const FunctionCallbackInfo<Value>& args) {
    uint16_t* inBuf = NULL;
    size_t inBufSize = 0;
    if (args.Length() >= 2) {
      inBuf = static_cast<uint16_t*>(  // NULL on flush.
...
Handle<Value> AccentedCharsStripFunc(const FunctionCallbackInfo<Value>& args) {
    uint16_t* inBuf = NULL;
    size_t inBufSize = 0;
    if (args.Length() >= 2) {
      inBuf = static_cast<uint16_t*>(  // NULL on flush.
